...
spaces_list = session.query(Space).all()
space_id = spaces_list[0].id if spaces_list else 1
...
test_username = f"testfinal_{unique_id}"
test_email = f"final_{unique_id}@example.com"
test("Signup", "POST", "/auth/signup", {"username":test_username,"password":"test1234","passwordconfirm":"test1234","email":test_email})
r = test("Login", "POST", "/auth/login", {"username":test_username,"password":"test1234"})
token = r.get_json()['token'] if r and r.status_code == 200 else None
h = {"Authorization": f"Bearer {token}"} if token else {}

# Build admin token from DB secret
admin_user = session.query(User).filter(User.role == 'admin').first()
if admin_user:
    import jwt as pyjwt
    admin_token = pyjwt.encode({'user_id': admin_user.id, 'role': 'admin', 'exp': 9999999999}, app.config['SECRET_KEY'], algorithm='HS256')
    h_admin = {"Authorization": f"Bearer {admin_token}"}
else:
    h_admin = h

# === 2. COURSES (run early, before session gets dirty) ===
print("\n--- 2. Courses ---")
test("List courses", "GET", "/courses/")
r = test("Create course", "POST", "/courses/", {"course_name":f"Film_{unique_id}","description":"Intro","status":"active","start_date":"2027-11-01","end_date":"2027-12-01"})
c_id = r.get_json()['id'] if r and r.status_code == 201 else None
if c_id:
    test("Get course", "GET", f"/courses/{c_id}")
    test("Update course", "PUT", f"/courses/{c_id}", {"course_name":f"Film_Upd_{unique_id}","description":"Updated","status":"active","start_date":"2027-11-01","end_date":"2027-12-01"})
    test("Delete course", "DELETE", f"/courses/{c_id}")

# === 3. ROOMS ===
print("\n--- 3. Rooms ---")
test("List rooms", "GET", "/rooms/")
r = test("Create room", "POST", "/rooms/", {"name":f"TestRoom_{unique_id}","room_type":"studio","capacity":10,"price_per_hour":200000,"status":"available"})
room_id = r.get_json()['id'] if r and r.status_code == 201 else None
if room_id:
    test("Get room", "GET", f"/rooms/{room_id}")
    test("Update room", "PUT", f"/rooms/{room_id}", {"name":f"TestRoom_Upd_{unique_id}","room_type":"studio","capacity":15,"price_per_hour":250000,"status":"available"})
...
r = test("Create space", "POST", "/spaces/", {"provider_id":provider_id,"name":f"Space_{unique_id}","space_type":"darkroom","description":"Test space","address":"123 Test","max_capacity":5,"base_price_per_hour":150000,"status":True}, h_admin)
new_space_id = r.get_json()['id'] if r and r.status_code == 201 else None
if new_space_id:
    test("Get space", "GET", f"/spaces/{new_space_id}")
    test("Update space", "PUT", f"/spaces/{new_space_id}", {"provider_id":provider_id,"name":f"Space_Upd_{unique_id}","space_type":"darkroom","base_price_per_hour":180000}, h_admin)
    # Images
    ...
    test("Delete space", "DELETE", f"/spaces/{new_space_id}", None, h_admin)

# === 4. EQUIPMENT ===
print("\n--- 4. Equipment ---")
test("List equipment", "GET", "/api/v1/equipment")
r = test("Create equipment", "POST", "/api/v1/equipment", {"provider_id":provider_id,"name":f"CanonR5_{unique_id}","type":"camera","condition":"excellent","price_per_hour":100000})
eq_id = r.get_json()['id'] if r and r.status_code == 201 else None
if eq_id:
    test("Get equipment", "GET", f"/api/v1/equipment/{eq_id}")
    test("Update equipment", "PUT", f"/api/v1/equipment/{eq_id}", {"name":f"CanonR5_Upd_{unique_id}","type":"camera","condition":"good","price_per_hour":120000})
...
# Use unique future date to avoid collision
rsv_year = 2030
rsv_day = (unique_id % 28) + 1
r = test("Create reservation", "POST", "/v1/reservations/", {
    "user_id": user_id_for_test, "provider_id": provider_id, "space_id": space_id,
    "start_time": f"{rsv_year}-01-{rsv_day:02d}T09:00:00", "end_time": f"{rsv_year}-01-{rsv_day:02d}T11:00:00", "total_price": 300000
}, h_admin)
...
if res_id:
    ...
    test("Approve", "POST", f"/v1/reservations/{res_id}/approve", None, h_admin)
    test("Confirm", "POST", f"/v1/reservations/{res_id}/confirm", None, h_admin)
    test("Create payment", "POST", f"/v1/reservations/{res_id}/payment", {"user_id":user_id_for_test,"amount":300000,"method":"cash"}, h_admin)
    test("Get payment", "GET", f"/v1/reservations/{res_id}/payment")
    test("List reviews", "GET", f"/v1/reservations/{res_id}/reviews")
    test("Add review", "POST", f"/v1/reservations/{res_id}/reviews", {"user_id":user_id_for_test,"space_id":space_id,"rating":5,"comment":"Great!"}, h_admin)
    test("Checkin", "POST", f"/v1/reservations/{res_id}/checkin", None, h_admin)
    test("Checkout", "POST", f"/v1/reservations/{res_id}/checkout", None, h_admin)
...
r = test("Create customer", "POST", "/v1/billing/customers", {"customer_name":f"Cust_{unique_id}","email":f"cf_{unique_id}@x.com","phone":"0900000000"}, h)
cust_id = r.get_json()['id'] if r and r.status_code == 201 else None
if cust_id:
    test("Get customer", "GET", f"/v1/billing/customers/{cust_id}")
    test("Update customer", "PUT", f"/v1/billing/customers/{cust_id}", {"customer_name":f"Cust_Upd_{unique_id}"}, h)
# Products
test("List products", "GET", "/v1/billing/products")
r = test("Create product", "POST", "/v1/billing/products", {"product_name":f"Film_{unique_id}","description":"35mm","product_code":f"FF{unique_id}"}, h)
prod_id = r.get_json()['id'] if r and r.status_code == 201 else None
if prod_id:
    test("Get product", "GET", f"/v1/billing/products/{prod_id}")
    test("Update product", "PUT", f"/v1/billing/products/{prod_id}", {"product_name":f"Film_Upd_{unique_id}"}, h)
# Invoices
...
if inv_id:
    ...
    if prod_id:
        test("Add invoice item", "POST", f"/v1/billing/invoices/{inv_id}/items", {"product_id":prod_id,"quantity":2,"unit_price":250000}, h)
...
if packages:
    ...
    r = test("Create booking", "POST", "/api/v1/package-bookings", {
        ...
        "start_time": "2027-06-20T09:00:00", "end_time": "2027-06-20T12:00:00", "notes": "Test"
    })
