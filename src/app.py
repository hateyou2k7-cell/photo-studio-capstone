...
async function createSpace(){const d={provider_id:3,name:document.getElementById('sp-name').value,space_type:document.getElementById('sp-type').value,base_price_per_hour:parseFloat(document.getElementById('sp-price').value),max_capacity:parseInt(document.getElementById('sp-cap').value),status:true};if(!d.name){showMsg('Nhap ten!',false);return}const{ok,data}=await apiCall('POST','/spaces/',d,true);showMsg(ok?'Tao thanh cong: '+data.name:(data.message||'Loi'),ok);if(ok)loadSpaces()}
...
async function createEquipment(){const d={provider_id:3,name:document.getElementById('eq-name').value,type:document.getElementById('eq-type').value,price_per_hour:parseFloat(document.getElementById('eq-price').value),condition:'good',is_available:true};if(!d.name){showMsg('Nhap ten!',false);return}const{ok,data}=await apiCall('POST','/api/v1/equipment',d,true);showMsg(ok?'Tao thanh cong: '+data.name:(data.message||'Loi'),ok);if(ok)loadEquipment()}
