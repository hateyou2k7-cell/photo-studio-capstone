...
// SPACES
async function loadSpaces(){const{data}=await apiCall('GET','/spaces/',null,true);const tb=document.getElementById('spl');tb.innerHTML='';(Array.isArray(data)?data:[]).forEach(r=>{const tr=document.createElement('tr');tr.innerHTML='<td>'+r.id+'</td><td>'+r.name+'</td><td>'+(r.space_type||r.type||'-')+'</td><td>'+Number(r.base_price_per_hour||0).toLocaleString()+'</td><td>'+r.max_capacity+'</td><td>'+(r.status?'Active':'Inactive')+'</td><td><button class="btn btn-red" onclick="delSpace('+r.id+')">Xoa</button></td>';tb.appendChild(tr)});if(!Array.isArray(data)||!data.length)tb.innerHTML='<tr><td colspan="7" style="text-align:center;color:#666">Khong co space</td></tr>'}
async function createSpace(){const d={provider_id:6,name:document.getElementById('sp-name').value,space_type:document.getElementById('sp-type').value,base_price_per_hour:parseFloat(document.getElementById('sp-price').value),max_capacity:parseInt(document.getElementById('sp-cap').value),status:true};if(!d.name){showMsg('Nhap ten!',false);return}const{ok,data}=await apiCall('POST','/spaces/',d,true);showMsg(ok?'Tao thanh cong: '+data.name:(data.message||'Loi'),ok);if(ok)loadSpaces()}
